    ...
    // 一般使用 影片的id寫在js裡
	 player = new YT.Player('slipinmv', {
	  ...
	  playerVars: {
		  ...
		  loop: 0,                // 重覆播放
		  playlist:'EXOKI2pCp7g', // 當使用影片要重覆播放時，需再輸入YouTube 影片ID
      },
      events: {
        'onReady': onPlayerReady
      }
    });
	 player = new YT.Player('slipinmv-kid', {
      height: '100%',
      width: '100%',
      videoId: 'gjC-WFt8buA',
	  playerVars: {
      ...
      autoplay:0,
		  loop: 0,                // 重覆播放
		  playlist:'gjC-WFt8buA', // 當使用影片要重覆播放時，需再輸入YouTube 影片ID
      },
      events: {
        'onReady': onPlayerReady
      }
